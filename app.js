let songs = [
    {
       name: 'Hey Brother',
       path: 'musics/1.mp3',
       artist: 'Avicii,Dan Tyminski',
       cover: 'images/1.jpeg'
    },
    {
        name: 'Addicted To You',
        path: 'musics/2.mp3',
        artist: 'Avicii,Audra Mae',
        cover: 'images/2.jpeg'
     },
     {
        name: 'Broken Arrows',
        path: 'musics/3.mp3',
        artist: 'Avicii',
        cover: 'images/3.jpeg'
     },
     {
        name: 'Dear Boy',
        path: 'musics/4.mp3',
        artist: 'Avicii',
        cover: 'images/4.jpeg'
     },
     {
        name: 'Fade Into Darkness',
        path: 'musics/5.mp3',
        artist: 'Avicii',
        cover: 'images/5.jpeg'
     },
     {
        name: 'Fades Away',
        path: 'musics/6.mp3',
        artist: 'Avicii,Nonnie Bao',
        cover: 'images/6.jpeg'
     },
     {
        name: 'For A Better Day',
        path: 'musics/7.mp3',
        artist: 'Avicii',
        cover: 'images/7.jpeg'
     },
     {
        name: 'Gonna Love Ya',
        path: 'musics/8.mp3',
        artist: 'Avicii',
        cover: 'images/8.jpeg'
     },
     {
        name: 'Heaven',
        path: 'musics/9.mp3',
        artist: 'Avicii,Chris Martin',
        cover: 'images/9.jpeg'
     },
     {
        name: 'Levels',
        path: 'musics/10.mp3',
        artist: 'Avicii',
        cover: 'images/10.jpeg'
     },
     {
        name: 'Lonely Together',
        path: 'musics/11.mp3',
        artist: 'Avicii,Rita Ora',
        cover: 'images/11.jpeg'
     },
     {
        name: 'Pure Grinding',
        path: 'musics/12.mp3',
        artist: 'Avicii',
        cover: 'images/12.jpeg'
     },
     {
        name: 'Silhouettes',
        path: 'musics/13.mp3',
        artist: 'Avicii,Salem Al Fakir',
        cover: 'images/13.jpeg'
     },
     {
        name: 'The Nights',
        path: 'musics/14.mp3',
        artist: 'Avicii',
        cover: 'images/14.jpeg'
     }, 
     {
        name: 'Tough Love',
        path: 'musics/15.mp3',
        artist: 'Avicii,Agnes,Vargas & Lagola',
        cover: 'images/15.jpeg'
     },
     {
        name: 'Waiting For Love',
        path: 'musics/16.mp3',
        artist: 'Avicii,Martin Garrix',
        cover: 'images/16.jpeg'
     },
     {
        name: 'Wake Me Up',
        path: 'musics/17.mp3',
        artist: 'Avicii,Aloe Blacc',
        cover: 'images/17.jpeg'
     },
     {
        name: 'Without You',
        path: 'musics/18.mp3',
        artist: 'Avicii,Sandro Cavazza',
        cover: 'images/18.jpeg'
     },
     {
        name: 'You Make Me',
        path: 'musics/19.mp3',
        artist: 'Avicii',
        cover: 'images/19.jpeg'
     },
     {
        name: 'SOS',
        path: 'musics/20.mp3',
        artist: 'Avicii,Aloe Blacc',
        cover: 'images/20.jpeg'
     },
     {
        name: 'Back Where I Belong',
        path: 'musics/21.mp3',
        artist: 'Avicii,Otto Knows,LP',
        cover: 'images/21.jpeg'
     },
     {
        name: 'Seek Bromance',
        path: 'musics/22.mp3',
        artist: 'Avicii',
        cover: 'images/22.jpeg'
     },
     {
        name: 'Live Your Life',
        path: 'musics/23.mp3',
        artist: 'Avicii,Afrojack,David Guetta,Ne-Yo',
        cover: 'images/23.jpeg'
     },
     {
      name: 'The Days',
      path: 'musics/24.mp3',
      artist: 'Avicii',
      cover: 'images/14.jpeg'
   },
   {
      name: 'What Would I Change It To',
      path: 'musics/25.mp3',
      artist: 'Avicii,Aluna',
      cover: 'images/18.jpeg'
   },
   {
      name: 'Forever Yours - Avicii Tribute',
      path: 'musics/26.mp3',
      artist: 'kygo,Avicii,Sandro Cavazza',
      cover: 'images/25.jpeg'
   },
   {
      name: 'New New New - Avicii Meets Yellow Remix',
      path: 'musics/27.mp3',
      artist: 'Bob Sinclar,Avicii',
      cover: 'images/26.jpeg'
   },
   {
      name: 'We Burn (Faster Than Light)',
      path: 'musics/28.mp3',
      artist: 'Avicii,Sandro Cavazza',
      cover: 'images/24.jpeg'
   },
   {
      name: 'Our Love',
      path: 'musics/29.mp3',
      artist: 'Avicii,Sandro Cavazza',
      cover: 'images/27.jpeg'
   }





]